...
const Sidebar = () => {
  ...
  // On mobile: Fixed position sidebar that slides in/out
  // On desktop: Static position sidebar that's always visible
  const sidebarClass = `h-full w-72 border-r border-base-300 flex flex-col bg-base-100 
    lg:static lg:block
    ${!selectedUser ? 'flex' : 'fixed transform transition-transform duration-300 z-40 lg:translate-x-0 ' + 
    (isMobileMenuOpen ? 'translate-x-0' : '-translate-x-full')}`;
    
  const overlayClass = `fixed inset-0 bg-black bg-opacity-50 z-30 transition-opacity duration-300
    ${isMobileMenuOpen && selectedUser ? 'opacity-100' : 'opacity-0 pointer-events-none'} lg:hidden`;

  // Only show mobile menu button when a chat is selected
  const showMobileMenuButton = selectedUser;

  return (
    <>
      {/* Mobile menu toggle button - only visible when chat is selected */}
      {showMobileMenuButton && (
        <button 
          onClick={toggleMobileMenu}
          className="lg:hidden fixed top-24 left-4 z-50 bg-primary text-primary-content p-2 rounded-full shadow-lg"
        >
      ...
      {/* Mobile overlay */}
      <div className={overlayClass} onClick={toggleMobileMenu}></div>
